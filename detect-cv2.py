...
def worker():
    ...
    while True:
        ...
        input_tensor = resized_frame[np.newaxis, ...]
        
        # Get the required input type from the model
        input_type = input_details[0]['dtype']
        
        if input_type == np.uint8:
            # Model expects 0 to 255
            input_tensor = input_tensor.astype(np.uint8)
            
        elif input_type == np.int8:
            # Model expects -128 to 127
            # We must shift the 0-255 uint8 data to fit the int8 range
            input_tensor = (input_tensor.astype(np.int32) - 128).astype(np.int8)
            
        elif input_type == np.float32:
            # Model expects 0.0 to 1.0 (or similar)
